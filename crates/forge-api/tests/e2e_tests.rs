...
async fn test_full_session_flow() {
    ...
    assert!(read_body["output"].as_str().unwrap().contains("Hello from E2E test!"));
...
async fn test_git_clone_and_modify() {
    ...
    assert!(status == 201 || status == 500, "Profile creation should either succeed or fail gracefully");
...
async fn test_error_handling() {
    ...
    assert!(tool_resp1.status() == 400 || tool_resp1.status() == 422, 
            "Invalid session ID should return 400 or 422, got {}", tool_resp1.status());
    ...
    assert!(tool_resp2.status() == 404 || tool_resp2.status() == 422,
            "Non-existent session should return 404 or 422, got {}", tool_resp2.status());
    ...
    assert!(session_resp.status() == 404 || session_resp.status() == 422,
            "Non-existent profile should return 404 or 422, got {}", session_resp.status());
    ...
    if invalid_resp.status() == 401 {
        ...
        println!("Note: Invalid API key returned {}, expected 401", invalid_resp.status());
    }
    ...
    assert!(profile_resp.status() == 400 || profile_resp.status() == 422 || profile_resp.status() == 500,
            "Missing fields should return 4xx/5xx, got {}", profile_resp.status());
