...
use forge_api::db::{CreateProfile, CreateUser, LoginRequest};
use serde_json::json;
...
async fn test_register_duplicate_email() {
    ...
    assert_eq!(resp2.status(), 409, "Duplicate email should return 409 Conflict");
}
...
async fn test_login_success() {
    ...
    let body: serde_json::Value = resp.json().await.unwrap();
    assert!(body["api_key"].is_string(), "Response should contain api_key");
...
async fn test_create_profile() {
    ...
    let body: serde_json::Value = resp.json().await.unwrap();
    assert!(body["profile"]["id"].is_string(), "Response should contain profile ID");
...
async fn test_list_profiles() {
    ...
    let body: serde_json::Value = resp.json().await.unwrap();
    assert!(body["profiles"].is_array(), "Response should contain profiles array");
    assert!(!body["profiles"].as_array().unwrap().is_empty(), "Should have at least one profile");
}
...
async fn test_create_session() {
    ...
    let body: serde_json::Value = resp.json().await.unwrap();
    assert!(body["session"]["id"].is_string(), "Response should contain session ID");
    assert_eq!(body["session"]["title"], "Test Session");
    assert!(body["working_dir"].is_string(), "Response should contain working_dir");
}
...
async fn test_list_sessions() {
    ...
    let body: serde_json::Value = resp.json().await.unwrap();
    assert!(body["sessions"].is_array(), "Response should contain sessions array");
    assert!(body["sessions"].as_array().unwrap().len() >= 3, "Should have at least 3 sessions");
}
...
async fn test_list_messages() {
    ...
    let body: serde_json::Value = resp.json().await.unwrap();
    assert!(body["messages"].is_array(), "Response should contain messages array");
}
...
async fn test_metrics_endpoint() {
    ...
    let body: serde_json::Value = resp.json().await.unwrap();
    assert!(body["metrics"].is_object(), "Response should contain metrics");
    assert!(body["error_rate"].is_string(), "Response should contain error_rate");
}
...
async fn test_prometheus_metrics() {
    ...
    let text = resp.text();
    assert!(text.contains("forge_requests_total"), "Should contain request metrics");
