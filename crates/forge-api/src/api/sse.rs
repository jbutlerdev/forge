...
/// 
/// This function spawns a bash process and streams stdout/stderr as SSE events.
/// It supports timeout and nix-shell wrapping.
    ...
    match tool_name {
        "bash" => {
            ...
            Ok(execute_bash_streaming(
                ...
                String::new(), // tool_call_id will be added by the handler
            ).await)
        }
        ...
        _ => {
            ...
            tokio::spawn(async move {
                ...
                let executor = ToolExecutor::new_with_nix(working_dir, false, nix_shell);
                match executor.execute(&tool_name_owned, input).await {
                    Ok(output) => {
...
pub async fn stream_tool_execution(
    ...
    // Get the session's isolated working directory
    ...
    match execute_streaming_tool(&working_dir, &payload.tool, payload.input, nix_shell.as_deref()).await {
        Ok(stream) => {
