...
if [ -d "$BUILD_OUT/etc/ssl/certs" ]; then
    ...
    # Python's ssl + many tools (httpx/requests, git, curl depend-
    # ing on backend) look up the Debian-standard CA path
    # /etc/ssl/certs/ca-certificates.crt by default when no
