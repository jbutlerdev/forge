...
const SpawnInputSchema = Type.Object({
    kind: Type.Optional(Type.String({ description: "agent kind: forge (default) or pi" })),
    name: Type.Optional(Type.String({ description: "optional session name (mode=session)" })),
    prompt: Type.String({ description: "first prompt for the new agent" }),
    mode: Type.Optional(Type.String({ description: "\"split\" (default) anchors a new pane next to you; \"session\" creates a named session", enum: ["split", "session"] })),
    profile_id: Type.Optional(Type.String({ description: "forge agent profile to use (kind=forge)" })),
});

const SendInputSchema = Type.Object({
    pane: Type.String({ description: "target pane id (from ranch_spawn)" }),
    text: Type.String({ description: "the message" }),
    delivery: Type.Optional(Type.String({ description: "\"steer\" redirects the agent ASAP; \"queue\" delivers after its turn", enum: ["steer", "queue"] })),
});

const PaneInputSchema = Type.Object({
    pane: Type.String({ description: "target pane id" }),
});

const ReadInputSchema = Type.Object({
    pane: Type.String({ description: "target pane id" }),
    since_seq: Type.Optional(Type.Number({ description: "only rows after this seq" })),
    limit: Type.Optional(Type.Number({ description: "max rows (default 50)" })),
});

async function callRanchTool(tool: string, params: Record<string, any>): Promise<ToolCallResult> {
...
async function ranchToolsExtension(pi: any) {
    if ((pi as any).config?.forgeApiUrl) forgeApiUrl = (pi as any).config.forgeApiUrl;
    if ((pi as any).config?.sessionId) sessionId = (pi as any).config.sessionId;

    const defs = [
        {
            name: "ranch_spawn",
            description:
                "Spawn a sub-agent in a new ranch pane and delegate a task to it. " +
            ...
            execute: async (_id: string, params: any) => callRanchTool("ranch_spawn", params),
        },
        {
            name: "ranch_send",
            description:
        ...
        },
        {
            ...
            parameters: PaneInputSchema,
            execute: async (_id: string, params: any) => callRanchTool("ranch_close", params),
        },
    ];
